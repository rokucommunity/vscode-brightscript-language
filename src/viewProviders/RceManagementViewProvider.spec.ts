...
describe('RceManagementViewProvider', () => {
    ...
    describe('updateRceDevice', () => {
        it('calls updateDevice with the new name and note', async () => {
            ...
            expect(updateDeviceArgs[0].deviceId).to.equal(5);
            expect(updateDeviceArgs[0].update.name).to.equal('renamed');
            expect(updateDeviceArgs[0].update.note).to.equal('new note');
        });
    });
    ...
    describe('deleteRceSnapshot', () => {
        ...
        it('calls deleteSnapshot when the deletion is confirmed', async () => {
            ...
            expect(rceManager.fakeManagementClient.deleteSnapshot.calledWith({ deviceId: 5, snapshotId: 20 })).to.be.true;
        });
    ...
    describe('enableRceDevMode', () => {
        ...
        it('sends the developer-settings combo to a running device with an instance api url', async () => {
            ...
            expect(sendDeveloperSettingsCombo.calledOnce).to.be.true;
            expect(sendDeveloperSettingsCombo.firstCall.args[0]).to.eql({
                device: {
    ...
    describe('createRceSnapshot', () => {
        it('creates the snapshot and responds with it', async () => {
            ...
            expect(createSnapshotArgs[0].deviceId).to.equal(5);
            expect(createSnapshotArgs[0].snapshot).to.eql({ name: 'before-testing', note: 'clean install' });
    ...
    describe('watchRceDevice', () => {
        it('opens the video editor tab command with the device id and name', async () => {
            rceManager = new TestRceManager(vscode.context as any);
            await rceManager.addAccount('work', 'token-work');
            rceManager.fakeManagementClient.listDevices.resolves([]);
            ...
            const message = { command: ViewProviderCommand.watchRceDevice, context: { deviceId: 5, deviceName: 'my-device' } };
            ...
            expect(executeCommand.getCall(0).args).to.eql([VscodeCommand.rceWatchDeviceInEditor, 5, 'my-device']);

            const responseMessage = findResponseMessage(ViewProviderCommand.watchRceDevice);
            expect(responseMessage.response.success).to.be.true;
        });

        it('responds with an error when the editor command rejects', async () => {
            rceManager = new TestRceManager(vscode.context as any);
            await rceManager.addAccount('work', 'token-work');
            rceManager.fakeManagementClient.listDevices.resolves([]);
            sinon.stub(vscode.commands, 'executeCommand').rejects(new Error('editor exploded'));
            ...
            const responseMessage = findResponseMessage(ViewProviderCommand.watchRceDevice);
            expect(responseMessage.error.message).to.equal('editor exploded');
        });
