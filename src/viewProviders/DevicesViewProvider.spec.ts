...
describe('DevicesViewProvider', () => {
    ...
    describe('restartDevice', () => {
        it('should prompt for confirmation before restarting', async () => {
            const device = makeDevice({ key: 'device1' });
            const { provider } = createProvider([device]);
            const element: any = { key: 'device1', contextValue: 'device' };
            ...
            await provider.restartDevice(element);

            expect(showWarningMessageStub.calledOnce).to.be.true;
            expect(showWarningMessageStub.firstCall.args[0]).to.include('Are you sure you want to restart');
        });

        it('should show error if device not found', async () => {
            const { provider } = createProvider([]);
            const element: any = { key: 'nonexistent', contextValue: 'device' };
            ...
            expect(showErrorMessageStub.firstCall.args[0]).to.equal('Device not found');
        });

        it('should show error if element is invalid', async () => {
            const device = makeDevice({ key: 'device1' });
            const { provider } = createProvider([device]);

            const showErrorMessageStub = sinon.stub(vscode.window, 'showErrorMessage').resolves();

            await provider.restartDevice(null as any);

            expect(showErrorMessageStub.calledOnce).to.be.true;
            expect(showErrorMessageStub.firstCall.args[0]).to.equal('Please select a device to restart');
        });
    });

    describe('checkForUpdates', () => {
        it('should prompt for confirmation before checking for updates', async () => {
            const device = makeDevice({ key: 'device1' });
            const { provider } = createProvider([device]);
            const element: any = { key: 'device1', contextValue: 'device' };
            ...
            await provider.checkForUpdates(element);

            expect(showInformationMessageStub.calledOnce).to.be.true;
            expect(showInformationMessageStub.firstCall.args[0]).to.include('Check for software updates');
        });
        ...
        it('should show error if element is invalid', async () => {
            ...
            await provider.checkForUpdates(null as any);

            expect(showErrorMessageStub.calledOnce).to.be.true;
            expect(showErrorMessageStub.firstCall.args[0]).to.equal('Please select a device to check for updates');
        });
    });
});
