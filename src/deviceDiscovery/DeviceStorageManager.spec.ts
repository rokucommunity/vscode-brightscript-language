...
describe('DeviceStorageManager', () => {
    let manager: DeviceStorageManager;
    let mockContext: any;
    ...
    beforeEach(() => {
        ...
        manager = new DeviceStorageManager(mockContext);
    });
    ...
    describe('getSerialNumberForIp', () => {
        ...
        it('returns undefined when IP not found in current network (strict lookup)', () => {
            manager.setSerialNumberForIp(networkB, '10.0.2.200', 'XYZ789');
            // networkA has no entry for this IP - strict lookup returns undefined
            expect(manager.getSerialNumberForIp('10.0.2.200', networkA)).to.be.undefined;
        });

        it('falls back to another network when no network specified', () => {
            manager.setSerialNumberForIp(networkB, '10.0.2.200', 'XYZ789');
            // No network specified - falls back to search all networks
            expect(manager.getSerialNumberForIp('10.0.2.200')).to.equal('XYZ789');
        });
    ...
    describe('heartbeat timestamps', () => {
        it('returns undefined when no timestamp stored', () => {
            expect(manager.getLastAliveTimestamp('device-123')).to.be.undefined;
        });

        it('stores and retrieves a timestamp', () => {
            const now = Date.now();
            manager.setLastAliveTimestamp('device-123', now);
            expect(manager.getLastAliveTimestamp('device-123')).to.equal(now);
        });

        it('keeps timestamps separate by key', () => {
            const now = Date.now();
            manager.setLastAliveTimestamp('device-123', now);
            manager.setLastAliveTimestamp('device-456', now + 1000);
            expect(manager.getLastAliveTimestamp('device-123')).to.equal(now);
            expect(manager.getLastAliveTimestamp('device-456')).to.equal(now + 1000);
        });

        it('overwrites existing timestamp', () => {
            const now = Date.now();
            manager.setLastAliveTimestamp('device-123', now);
            manager.setLastAliveTimestamp('device-123', now + 5000);
            expect(manager.getLastAliveTimestamp('device-123')).to.equal(now + 5000);
        });
    });
});
