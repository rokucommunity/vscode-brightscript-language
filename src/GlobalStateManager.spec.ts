...
describe('GlobalStateManager', () => {
    ...
    describe('knownDeviceIps', () => {
        it('returns empty array when no IPs stored', () => {
            expect(manager.knownDeviceIps).to.deep.equal([]);
        });

        it('adds a new IP', () => {
            manager.addKnownDeviceIp('192.168.1.100');
            expect(manager.knownDeviceIps).to.deep.equal(['192.168.1.100']);
        });
        ...
        it('removes an IP', () => {
            ...
            expect(manager.knownDeviceIps).to.deep.equal(['192.168.1.101']);
        });
    });
});
